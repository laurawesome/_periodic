...
static NSString * const k_periodic_mobileJavascript = @"Ti.App={};Ti.API={};Ti.App._listeners={};Ti.App._listener_id=1;Ti.App.id=Ti.appId;Ti.App._xhr=XMLHttpRequest;"
...
-(NSString*)__periodic_mobileInjection
{
	...
	[html appendString:k_periodic_mobileJavascript];
...
{
	// attempt to make well-formed HTML and inject in our _periodic_mobile bridge code
	...
	if (range.location!=NSNotFound)
	{
		...
		if (nextRange.location!=NSNotFound)
		{
			...
			[html appendString:[self __periodic_mobileInjection]];
	...
	else
	{
		...
		if ([[NSFileManager defaultManager] fileExistsAtPath:path])
		{
			...
			else
			{
				...
				{
					NSLog(@"[WARN] I have no idea what the appropriate text encoding is for: %@. Please report this to _periodic_mobile support.",url);
				}
			}
